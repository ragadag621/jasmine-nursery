...
export function ReviewsSection() {
  ...
  return (
    <section className="bg-[var(--color-sage-100)] px-4 py-16 md:px-6">
      <div className="mx-auto max-w-6xl">
        <div className="mb-10 text-center">
          <h2 className="font-display mb-2 text-2xl text-[var(--color-forest-800)] md:text-3xl">
            מה הלקוחות אומרים
          </h2>
          {content && (
            <p className="text-[var(--color-ink-600)]">
              ...
              · {content.googleReviewCount} ביקורות בגוגל
            </p>
          )}
        </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {testimonials.map((t) => (
              <Card key={t._id} className="p-6">
                ...
                <p className="mt-2 text-[var(--color-ink-900)]">{t.text.he}</p>
                <p className="mt-3 text-sm font-medium text-[var(--color-ink-600)]">{t.customerName}</p>
              </Card>
            ))}
          </div>
        )}
